...
class MT5Engine:
    ...
    def rates(self, s, tf, n=100):
        ...
        return np.array([(x['time'],x['open'],x['high'],x['low'],x['close'],x['tick_volume'])
                        for x in r], dtype=[('t','i8'),('o','f8'),('h','f8'),('l','f8'),('c','f8'),('v','i8')]) if r is not None and len(r) > 0 else None
    ...
    def atr_history(self, h, l, c, p=14):
        """Calculate historical ATR array"""
        if len(h) < p+1: return []
        tr = []
    ...
    def levels(self, rates):
        ...
        return max(resist) if resist is not None and len(resist) > 0 else None, min(supp) if supp is not None and len(supp) > 0 else None
    ...
    def prev_close(self, sym):
        try:
            ...
            return r[0]["close"] if r is not None and len(r) > 0 else None
        except:
            return None
