...
class GoldAnalyzerApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(tf, text="\u26a1 HJ ANALYZER  v3.064 \u26a1", font=("Consolas", 14, "bold"),
    ...
    def _draw_chart(self, fig, a, title_prefix, chart_tv):
        ...
        # countdown display - 在标题栏显示倒计时
        cd = self.countdown_var.get()
        ak = "countdown_text_" + title_prefix.lower()
        # 清除旧的倒计时文本
        for child in ax.get_children():
            if hasattr(child, "get_text") and child.get_text() and "倒计时" in child.get_text():
                child.remove()
        # 添加新的倒计时文本
        countdown_text = ax.text(0.98, 0.95, f"倒计时: {cd}", transform=ax.transAxes,
                    fontsize=13, ha="right", va="top", color="#FFD700", fontweight="bold",
                    bbox=dict(boxstyle="round,pad=0.4", facecolor="#0a0a1a", edgecolor="#FFD700", linewidth=2, alpha=0.9))
    ...
    def _update_countdown(self):
        ...
        try:
            ...
            for prefix, tv_attr in [("m1", "chart_tv_m1"), ("h1", "chart_tv_h1")]:
                ...
                _dbg(f"Checking annotation: {ak}, found={an is not None}")
                if an is not None:
                    an.set_text(f"倒计时: {countdown_str}")
                    canvas_attr = "canvas_" + prefix
                    canvas = getattr(self, canvas_attr, None)
                    if canvas is not None:
                        canvas.draw_idle()
                        _dbg(f"Canvas drawn for {prefix}")
        except Exception as e:
