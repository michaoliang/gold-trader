...
class GoldAnalyzerApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(tf, text="\u26a1 HJ ANALYZER  v3.061 \u26a1", font=("Consolas", 14, "bold"),
    ...
    def _draw_chart(self, fig, a, title_prefix, chart_tv):
        ...
        # countdown annotation - 黑色背景白色字更明显
        cd = self.countdown_var.get()
        ak = 'countdown_annot_' + title_prefix.lower()
        an = getattr(self, ak, None)
        if an is not None:
            an.set_text(f'倒计时: {cd}')
        else:
            an = ax.annotate(f'倒计时: {cd}', xy=(0.98, 0.98), xycoords='axes fraction', fontsize=12,
                    ha='right', va='top', color='#FFFFFF', fontweight='bold',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='#000000', edgecolor='#ffaa00', alpha=0.8))
            setattr(self, ak, an)
