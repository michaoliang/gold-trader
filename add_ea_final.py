...
# Add EA log methods before _panel_alerts
ea_methods = '''    def _parse_ea_log(self):
        """解析EA日志并更新右侧面板"""
        try:
            import os
            ...
            display = (
                signal_text + "  [" + last_time + "]\\n"
                "版本: " + version + " | 时段: " + session + "\\n"
                "价格区间: " + low_price + " ~ " + high_price + "\\n"
                "距低点: " + price_from_low + "\\n"
                "MA1下跌: " + ma1_ok + "  MA6下跌: " + ma6_ok + "  DEMA: " + dema_ok + "\\n"
                "ATR许可: " + atr_ok + "  M1频率: " + freq1_ok + "  M6频率: " + freq6_ok
            )

            self._update_ea_display(display, signal_color)

        except Exception as e:
            self._update_ea_display("解析错误: " + str(e)[:30], "red")

    def _update_ea_display(self, text, color):
        """更新EA状态显示"""
        try:
...
# Insert before _panel_alerts
insert_point = content.find("    def _panel_alerts(self, parent):")
if insert_point < 0:
    print("ERROR: Could not find _panel_alerts")
    exit(1)

content = content[:insert_point] + ea_methods + content[insert_point:]

# Add EA log panel to _build_right_panels
old_end = "        self._panel_account(self.account_frame)\n\n    def _toggle_top"
new_end = "        self._panel_account(self.account_frame)\n\n        # EA日志状态面板\n        ea_fr = tk.Frame(self.account_frame, bg=self.C[\"card\"])\n        ea_fr.pack(fill=\"x\", padx=20, pady=(8, 4))\n        tk.Label(\n            ea_fr,\n            text=\"EA运行状态\",\n            font=(\"Consolas\", 9, \"bold\"),\n            fg=self.C[\"accent\"],\n            bg=self.C[\"card\"],\n        ).pack(anchor=\"w\")\n        self.ea_log_text = tk.Text(\n            ea_fr, height=6, font=(\"Consolas\", 8),\n            fg=self.C[\"tx\"], bg=self.C[\"card2\"],\n            insertbackground=self.C[\"tx\"],\n            relief=\"flat\", state=\"disabled\",\n            wrap=\"word\"\n        )\n        self.ea_log_text.pack(fill=\"x\", pady=(2, 0))\n\n    def _toggle_top"

if old_end in content:
    content = content.replace(old_end, new_end)
    print("Added EA log panel to right panel")
else:
    print("WARNING: Could not find insertion point for EA panel")
